...
import sst # speech to text module

class args:
    save_dir = 'reddit-model-1-5-18/reddit'
    n = 500
    prime = ' '
    beam_width = 2
    temperature = 1.1
...
def sample_main(args=args()):
    global net, sess, chars, vocab
...
def hello():
    global net, sess, chars, vocab
    request.files['wave_file'].save('tmp.wav')
    fs, data = client.convert_samplerate('tmp.wav') # downsampling original audio file to 16000
    
    # optional for debug and listening to audio after retrieve from client
    # import sounddevice as sd
    # sd.play(data, 16000)

    text = stt.main(data, 16000)

    out_chars = chatbot(net, sess, chars, vocab, args.n, args.beam_width,
            args.relevance, args.temperature, args.topn, text)
    response = Response(json.dumps({'response': ''.join(out_chars), 'request': text}), mimetype='application/json')
    return response

if __name__ == '__main__':
    args = args()
    sample_main(args=args)
